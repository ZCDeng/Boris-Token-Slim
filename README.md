...
Plus **15 gotcha detectors** that print actionable evidence (paths, names, line numbers):
...
| 15 | **Cache-prefix poisoning** | Volatile content (today's date, `# currentDate` headings, `last updated` stamps, recent-N injections) in `CLAUDE.md` / `MEMORY.md` that invalidates Anthropic's prompt-cache prefix — you pay full input price (10x cached read) on every session start. With $/month projection. |
